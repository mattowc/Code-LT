...
function delete_comment_link($id) {
    if (current_user_can('edit_post')) {
        echo '&nbsp;-&nbsp; <a href="'.admin_url("comment.php?action=cdc&c=$id").'">'.get_option('ptthemes_comment_delete_name').'</a> ';
        echo '&nbsp;-&nbsp; <a href="'.admin_url("comment.php?action=cdc&dt=spam&c=$id").'">'.get_option('ptthemes_comment_spam_name').'</a>';
    }
}
...
<li class="comment wrap threaded" id="comment-<?php	 	  comment_ID() ?>">
		    ...
		    <div class="meta-left">
				...
				<div class="meta-wrap">
					...
					<p class="authorcomment"><?php	 	  comment_author_link() ?> <br /> 
					
					<span> <?php	 	  if(!function_exists('how_long_ago')){comment_date('M d, Y'); } else { echo how_long_ago(get_comment_time('U')); } ?></span></p>
...
<?php	 	  comment_reply_link( array_merge( $args, array( 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
		...
		<?php	 	  edit_comment_link(''.get_option('ptthemes_comment_edit_name').'', '&nbsp;-&nbsp;&nbsp;', ''); ?>
